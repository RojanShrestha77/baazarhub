...
- **Policy (length + complexity):** enforced with Zod schemas **server-side** and mirrored **client-side** for fast feedback. *(Files: `backend/src/validators/auth.schema.ts`, frontend register/password pages)*
...
- **Strength feedback:** shown on the client during registration.
