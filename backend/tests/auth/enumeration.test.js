...
// whether an account exists — not via status code, not via body, not via
// headers. Currently failing: /login (and friends) are unimplemented
// stubs (501 with a debug-shaped body), not the generic contract below.
// This test encodes the target contract so it fails now and passes once
// that generic response is actually implemented.
...
describe("login — no user-existence signal", () => {
  it("returns identical status/body for a wrong password on an existing user vs a non-existent user", async () => {
    ...
    const nonexistent = await request(app)
      .post("/api/auth/login")
      .send({ email: "no-such-user@example.com", password: "definitely-wrong" });
    ...
    // Content-type/length should match too — a differing body length is
    // itself a side channel even if the visible text is generic-sounding.
...
describe("registration — no user-existence signal", () => {
  it("returns identical status/body whether or not the email is already registered", async () => {
    ...
    const takenRes = await request(app)
      ...
      .send({ email: user.email, password: "some-new-password-123" });

    const freeRes = await request(app)
      .post("/api/auth/register")
      .send({ email: "brand-new@example.com", password: "some-new-password-123" });
