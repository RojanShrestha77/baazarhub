    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center px-4">
        <div className="text-center max-w-sm">
          <div className="w-24 h-24 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-6"><MessageSquare className="w-12 h-12 text-indigo-300" /></div>
          ...
          <Link href="/marketplace" className="inline-flex items-center gap-2 bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-indigo-700 transition-colors shadow-sm">Browse Marketplace</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-[70vh]">
        ...
        <div className={`md:col-span-1 bg-white border border-gray-100 rounded-2xl shadow-sm overflow-y-auto ${activeId ? "hidden md:block" : ""}`}>
          {conversations.map((c) => (
            <button key={c.id} onClick={() => openThread(c.id)} className={`w-full text-left px-4 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors ${activeId === c.id ? "bg-indigo-50" : ""}`}>
            ...
            <>
                  ...
                  return (
                    <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"}`}>
                      <div className={`group max-w-[75%] flex flex-col ${mine ? "items-end" : "items-start"}`}>
                        <div className={`px-3.5 py-2 rounded-2xl text-sm ${mine ? "bg-indigo-600 text-white rounded-br-sm" : "bg-gray-100 text-gray-800 rounded-bl-sm"}`}>
                          {m.body}
                        </div>
              ...
              <div className="p-3 border-t border-gray-100 flex items-center gap-2">
                <input
                  ...
                  maxLength={2000}
                  className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 outline-none"
                />
                <button onClick={send} disabled={sending || !draft.trim()} aria-label="Send" className="bg-indigo-600 text-white p-2.5 rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-colors">
