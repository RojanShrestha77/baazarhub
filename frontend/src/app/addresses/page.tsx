...
export default function AddressesPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <Link href="/profile" className="inline-flex items-center gap-1 text-sm text-gray-400 hover:text-indigo-600 mb-6"><ChevronLeft className="w-4 h-4" /> Back to profile</Link>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {!showForm && (
          <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-1.5 bg-indigo-600 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-indigo-700 transition-colors">
      ...
      {showForm && (
        <motion.form initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} onSubmit={submit} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm mb-6 space-y-3">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-600">
            <input type="checkbox" checked={makeDefault} onChange={(e) => setMakeDefault(e.target.checked)} className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
            Set as default
          </label>
          <div className="flex justify-end gap-2 pt-1">
            ...
            <button type="submit" disabled={saving} className="bg-indigo-600 text-white px-5 py-2 rounded-xl text-sm font-semibold hover:bg-indigo-700 disabled:opacity-50">{saving ? "Saving…" : "Save address"}</button>
          </div>
        </motion.form>
      )}
            ...
            <div key={a.id} className="bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex items-start justify-between gap-4">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {a.isDefault && <span className="inline-flex items-center gap-1 text-xs bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full font-medium"><Star className="w-3 h-3 fill-current" /> Default</span>}
                </div>
              ...
              <div className="flex flex-col items-end gap-2 flex-shrink-0">
                {!a.isDefault && <button onClick={() => makeDefaultAddr(a.id)} className="text-xs text-indigo-600 hover:text-indigo-700 font-medium">Set default</button>}
...
function Field({ label, value, onChange, required }: { label: string; value: string; onChange: (v: string) => void; required?: boolean }) {
  return (
    <label className="block">
      ...
      <input value={value} onChange={(e) => onChange(e.target.value)} required={required} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 outline-none" />
    </label>
  );
}
