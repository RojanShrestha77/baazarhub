      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          {user && listing.sellerId !== user.id && (
            <div className="mt-6">
              {!contactOpen ? (
                <button onClick={() => setContactOpen(true)} className="inline-flex items-center gap-2 text-sm font-medium text-indigo-600 hover:text-indigo-700">
                ...
                <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
                  ...
                  <textarea value={contactMsg} onChange={(e) => setContactMsg(e.target.value)} maxLength={2000} rows={3} placeholder="Ask a question about this item…" className="w-full rounded-xl border border-gray-200 p-3 text-sm focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 outline-none resize-none" />
                  <div className="mt-2 flex justify-end gap-2">
                    ...
                    <button onClick={startConversation} disabled={contacting || !contactMsg.trim()} className="bg-indigo-600 text-white px-4 py-1.5 rounded-lg text-sm font-semibold hover:bg-indigo-700 disabled:opacity-50">{contacting ? "Sending…" : "Send"}</button>
                  </div>
                </div>
              )}
            </div>
        ...
        {user && (
          <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm mb-6">
            ...
            <textarea
              ...
              className="w-full rounded-xl border border-gray-200 p-3 text-sm text-gray-800 focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 outline-none resize-none"
            />
            <div className="mt-3 flex justify-end">
              <button onClick={submitReview} disabled={submitting} className="bg-indigo-600 text-white px-5 py-2 rounded-xl text-sm font-semibold hover:bg-indigo-700 disabled:opacity-50 transition-colors">
