                ...
                <div key={id} className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
                  <div className="flex items-center justify-between gap-4">
                    <Link href={`/listings/${id}`} className="flex-1 min-w-0 group">
                      <p className="font-medium text-gray-900 truncate group-hover:text-indigo-600 transition-colors">{l.title}</p>
                    ...
                    <span className="text-sm font-medium text-indigo-600 whitespace-nowrap">{l.priceMinorUnits !== undefined ? `NPR ${(l.priceMinorUnits / 100).toLocaleString()}` : ""}</span>
                  </div>
                ...
                <div key={o._id} className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
                  <div className="flex items-center justify-between gap-4">
                    <Link href={`/orders/${o._id}`} className="flex-1 min-w-0 group">
                      <p className="font-medium text-gray-900 truncate group-hover:text-indigo-600 transition-colors">{o.listingSnapshot?.title || "Order"}</p>
                      <p className="text-xs text-gray-400 mt-0.5 capitalize">{o.status.replace("_", " ")}</p>
                    </Link>
                    <div className="flex items-center gap-3">
                      {o.status === "payment_held" && <button disabled={busy} onClick={() => markShipped(o._id)} className="inline-flex items-center gap-1.5 text-xs bg-indigo-600 text-white px-3 py-1.5 rounded-lg font-medium hover:bg-indigo-700 disabled:opacity-50 transition-colors"><Truck className="w-3.5 h-3.5" />Mark Shipped</button>}
