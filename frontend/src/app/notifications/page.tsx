  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        {unread > 0 && (
          <button onClick={markAll} className="inline-flex items-center gap-1.5 text-sm text-indigo-600 hover:text-indigo-700 font-medium">
          ...
          {items.map((n, i) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => open(n)}
                className={`w-full text-left flex items-start gap-3 p-4 rounded-2xl border shadow-sm transition-colors ${n.read ? "bg-white border-gray-100 hover:bg-gray-50" : "bg-indigo-50/60 border-indigo-100 hover:bg-indigo-50"}`}
              >
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${n.read ? "bg-gray-100 text-gray-400" : "bg-indigo-100 text-indigo-600"}`}>
