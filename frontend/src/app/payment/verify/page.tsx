      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-sm bg-white border border-gray-100 rounded-2xl shadow-sm p-8">
        ...
        {state === "success" && (
          <>
            ...
            <button onClick={() => router.push(orderId ? `/orders/${orderId}` : "/orders")} className="inline-flex items-center gap-2 bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-indigo-700 transition-colors shadow-sm">View Order</button>
          </>
        )}
        {state === "error" && (
          <>
            ...
            <Link href="/cart" className="inline-flex items-center gap-2 bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-indigo-700 transition-colors shadow-sm">Back to Cart</Link>
          </>
        )}
      </motion.div>
    </div>
...
export default function PaymentVerifyPage() {
  return (
    <Suspense fallback={<div className="min-h-[70vh] flex items-center justify-center"><Loader2 className="w-8 h-8 text-indigo-500 animate-spin" /></div>}>
      <VerifyInner />
    </Suspense>
  );
}
